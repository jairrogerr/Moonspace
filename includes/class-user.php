...
class User
{
    ...
    public function sign_up($args = [])
    {
        ...
        /* register user */
        $db->query(sprintf("INSERT INTO users (user_name, user_email, user_phone, user_password, user_firstname, user_lastname, user_gender, user_cover, user_birthdate, user_registered, user_email_verification_code, user_phone_verification_code, user_privacy_newsletter) VALUES (%s, %s, %s, %s, %s, %s, %s, 'photos/cover_default.jpg', %s, %s, %s, %s, %s)", secure($args['username']), secure($args['email']), secure($args['phone']), secure(_password_hash($args['password'])), secure(ucwords($args['first_name'])), secure(ucwords($args['last_name'])), secure($args['gender']), secure($args['birth_date']), secure($date), secure($email_verification_code), secure($phone_verification_code), secure($newsletter_agree))) or _error("SQL_ERROR_THROWEN");
        /* get user_id */
        ...
        $db->query(sprintf("INSERT INTO followings (user_id, following_id, time) VALUES (%s, 1, %s)", secure($user_id, 'int'), secure($date))) or _error("SQL_ERROR_THROWEN");
