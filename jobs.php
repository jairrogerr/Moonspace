...
try {
	...
	switch ($_GET['view']) {
		...
		case 'category':
			...
			// page header
			page_header($system['system_title'] . ' - ' . __("Jobs") . ' - ' . __($current_category['category_name']), __($current_category['category_description']));
	...
	$total = $db->query("SELECT COUNT(*) as count " . $distance_clause . " FROM posts INNER JOIN posts_jobs ON posts.post_id = posts_jobs.post_id INNER JOIN users ON posts.user_id = users.user_id WHERE posts.post_type = 'job' AND posts_jobs.available = '1'" . $where_query . $distance_query . $order_query) or _error("SQL_ERROR");
	...
	// get posts
	$rows = array();
	$get_rows = $db->query("SELECT posts.post_id " . $distance_clause . " FROM posts INNER JOIN posts_jobs ON posts.post_id = posts_jobs.post_id INNER JOIN users ON posts.user_id = users.user_id WHERE posts.post_type = 'job' AND posts_jobs.available = '1'" . $where_query . $distance_query . $order_query . $limit_query) or _error("SQL_ERROR");
