...
try {
	...
	$total = $db->query("SELECT COUNT(*) as count " . $distance_clause . " FROM posts INNER JOIN posts_offers ON posts.post_id = posts_offers.post_id INNER JOIN users ON posts.user_id = users.user_id WHERE posts.post_type = 'offer' AND posts_offers.end_date >= CURDATE()" . $where_query . $distance_query . $order_query) or _error("SQL_ERROR");
	...
	// get posts
	$rows = array();
	$get_rows = $db->query("SELECT posts.post_id " . $distance_clause . " FROM posts INNER JOIN posts_offers ON posts.post_id = posts_offers.post_id INNER JOIN users ON posts.user_id = users.user_id WHERE posts.post_type = 'offer' AND posts_offers.end_date >= CURDATE()" . $where_query . $distance_query . $order_query . $limit_query) or _error("SQL_ERROR");
